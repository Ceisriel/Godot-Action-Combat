...
#mobile controls 		
	...
	if Input.is_action_pressed("guard") and not mousemode and not is_climbing and energy >= 0.125:
		...
		animation.play_backwards("run greatsword")	
		
	elif is_sprinting and not dodge:
		animation.play("run greatsword", 0, agility * 0.95)
	elif is_running:
		animation.play("sprint greatsword", 0, agility * 0.89)

	elif is_walking and is_on_floor():
		animation.play("run greatsword", 0.25)
	else:
		animation.play("t pose", 0.2)
