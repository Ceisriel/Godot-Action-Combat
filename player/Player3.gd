...
var player_state = GroundState.Grounded	
...
# Raycast to detect obstacles in front of the character
	...
	if Input.is_action_pressed("forward") or Input.is_action_pressed("backward") or Input.is_action_pressed("left") or Input.is_action_pressed("right"):
		...
		if Input.is_action_pressed("run") and is_walking and not is_climbing and not blocking:
			...
			is_sprinting = true
			enabled_climbing = false
		elif Input.is_action_pressed("crouch") and not is_climbing and not blocking and not is_running and not is_sprinting:
...
#Dodge forward
	...
	if dash_count == 2 and dash_timer < double_press_time and energy >= 1.25:
		horizontal_velocity = direction * dash_power * 4
		energy -= 0.125
		setStateSlide()
		dodge = true 
	else:
...
#Dodge right	
	...
	if Input.is_action_pressed("crouch"):
		collision.shape.height -= crouching_speed * delta
	else: 
		collision.shape.height += crouching_speed * delta
	collision.height= clamp(collision.shape.height, crouch_height, default_height)	
