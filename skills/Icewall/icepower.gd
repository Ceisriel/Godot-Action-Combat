...
onready var block_scene = load("res://skills/Icewall/Block.tscn")
...
func _process(_delta):
	if Input.is_action_just_pressed("icewall"):
		if not active:
			active = true
		else:
...
func _input(event):
	...
	if active and event is InputEventMouseMotion and  Input.is_action_just_pressed("guard"):
